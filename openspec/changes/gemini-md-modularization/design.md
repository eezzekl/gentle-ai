...
**Choice**: each component exports the managed reference path, and the path list derives from it. `engram.ManagedReferencePath(adapter any, injectionDir string) string` and `sdd.ManagedReferencePath(...)` return the absolute reference-file path, or `""` when no reference file is written for that target. They are the single source of truth for that decision: `Inject` writes exactly what they name, and `componentPathsWithWorkspaceScoped` lists exactly what they name, at the same two switch arms this change already touches:
- `case model.ComponentEngram:` — after the existing `StrategyMarkdownSections` append, append `engram.ManagedReferencePath(adapter, targetDir)` when non-empty.
- `case model.ComponentSDD:` — after `sddSubAgentPaths`, append `sdd.ManagedReferencePath(adapter, targetDir)` when non-empty.

**Why an exported helper instead of an inline type assertion**: the reference file is written only when the injection target is the home dir the stub advertises — a workspace-scoped install keeps the body inline and writes nothing (see `TestInjectKeepsProtocolInlineWhenTargetIsNotHome` and `TestInjectKeepsOrchestratorInlineWhenTargetIsNotHome`, both pre-existing). An unconditional append would therefore name two files that no workspace-scoped install ever writes, and post-apply and post-sync verification would fail on every one of them — turning a backup gap into a verification defect. Duplicating that home-anchoring condition at the call site would leave the writer and the path list free to drift apart, which is the exact failure class this decision exists to close, so the condition lives in one exported function that both sides call. `ManagedReferencePath` takes `any` and performs the `SharedReferenceLayout` assertion internally, so callers never name the interface.

**Interface visibility**: `SharedReferenceLayout` is not a single shared declaration. Per D1 it is declared locally in each consuming component — `internal/components/engram/inject.go` and `internal/components/sdd/inject.go`, structurally identical (`ReferencesDir(homeDir string) string`) — so neither name is in scope unqualified from package `cli`. That is why the assertion belongs inside each component's `ManagedReferencePath` rather than at the `run.go` call site, and why no third copy of the interface is declared in `cli`: it would add a fourth place to keep in sync for no gain.
...
**Testing**: `TestManagedReferencePathMatchesWhatInjectWrites` in each component pins the helper to the writer — for both adapters it asserts the returned path is the one `Inject` actually creates, that a workspace-scoped target returns `""`, and that an adapter without the layout returns `""`. `internal/cli/reference_backup_parity_test.go` then pins the consumers: `componentPathsWithWorkspaceScoped` includes both reference paths under global scope, omits them under workspace scope, and `syncBackupTargets` carries them through. The workspace-scope case is a regression guard, not a formality: it is the assertion that fails if the path list is ever wired unconditionally.
